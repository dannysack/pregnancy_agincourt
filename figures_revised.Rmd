...
library(rsvg)
loadfonts()
knitrSet(lang='markdown', fig.path='figures_rev/', fig.align='center', cache=TRUE)
options(prType='html')
```
...
fig1 <- grViz("digraph flowchart {
      ...
      mod3sesses [label = '(n = 25,524)']
      primexc [label = 'Missing birth year (n = 53) \\n Pre-1993 births (n = 950) \\n Post-2018 births (n = 1,510) \\n Duplicate recorded births (n = 685) \\n Delivery date on or after observation date (n = 124)']
      ...
      sensses [label = 'Socioeconomic status recorded\\n(n = 25,711)']
      ...
      edge[dir = none]
      all -> n1
      sens -> sens1 -> sens2 -> sens3 -> sens4 -> sens5 -> sensses
      mod1 -> mod1sens1 -> mod1sens2 -> mod1sens3 -> mod1sens4 -> mod1sens5 -> mod1sesses
      mod2 -> mod2sens1 -> mod2sens2 -> mod2sens3 -> mod2sens4 -> mod2sens5 -> mod2sesses
      mod3 -> mod3sens1 -> mod3sens2 -> mod3sens3 -> mod3sens4 -> mod3sens5 -> mod3sesses
      ...
      {rank = same; sensses; mod1sens; mod2sensses; mod3sensses}
      }
      ",
...
fig1 %>% 
  export_svg() %>%
  charToRaw() %>%
  rsvg_pdf("./figures_rev/fig1.pdf")
# embed fonts
embedFonts("./figures_rev/fig1.pdf", fontpaths = "/Library/Fonts/Microsoft/Arial.ttf")
```
...
# proportion of antenatal visits
...
ant_per <- ggplot(ant_df, aes(x = DeliveryYear, y = value, group = fct_rev(key), fill = key)) + 
  ...
  scale_fill_viridis_d(name = "Visits",
                      labels = c("0-3", 
                                 "4-7",
                                 "8+", "Missing")) +
  ylab("Proportion") + xlab("Delivery Year") + 
  ggtitle("") +
  guides(fill = guide_legend(nrow = 2)) +
  theme_pubr() + theme(legend.position = "bottom") +
  theme_fig2 + labs_pubr()
...
fig2 <- ggarrange(ant_per, loc_per, 
          att_per, ppcont_per,
          ncol = 2, nrow = 2,
          labels = c("a. Antenatal Visits",
                     "b. Delivery Location",
                     "c. Delivery Attendant",
                     "d. Intention/Current Postpartum Contraceptive Use"),
          font.label = list(size = 12),
          hjust = -0.05, vjust = 1.25)

fig2
ggsave("./figures_rev/fig2.pdf", width = 10, height = 8)
embed_fonts("./figures_rev/fig2.pdf")
```
...
fig3
ggsave("./figures_rev/fig3.pdf", width = 10, height = 5)
embed_fonts("./figures_rev/fig3.pdf")
```
...
fig4
ggsave("./figures_rev/fig4.pdf", width = 10, height = 8)
embed_fonts("./figures_rev/fig4.pdf")
```
...
int_per <- ggplot(int_df, aes(x = DeliveryYear, y = value, 
  ...
  scale_fill_viridis_d(name = "Intention",
                       labels = c("Unintended", "Other",
                                  "Intended", "Missing")) +
  ylab("Proportion of Pregnancies") + xlab("Delivery Year") + 
  ggtitle("") +
  theme_pubr() + theme(legend.position = "bottom") +
  theme_fig2 + labs_pubr()
...
```{r suppfig1, fig.width=10, fig.height=15}
supfig1 <- ggarrange(age_per, pcont_per, 
                     int_per, com_per,
                     edu_per, stu_per,
                     ncol = 2, nrow = 3,
                     labels = c("a. Mother's Age at Delivery",
                                "b. Pre-Pregnancy Contraceptive Use",
                                "c. Pregnancy Intention",
                                "e. Pregnancy Complications",
                                "e. Years of Education",
                                "f. Student Status During Pregnancy"),
                    font.label = list(size = 12),
...
```{r supfig5, fig.height=20, fig.width=10, warning=FALSE}
...
supfig2mod1 <- mod1s %>% rownames_to_column(., "Name") %>%
  ...
  left_join(., mod1sens5 %>% rownames_to_column(., "Name"), 
            by = c("Name", "Low", "High", "Diff.", "S.E.", "Type"))  %>%
  full_join(., mod1sensses %>% rownames_to_column(., "Name"), 
            by = c("Name", "Low", "High", "Diff.", "S.E.", "Type"))  %>%
  ...
  geom_point(aes(x = Effect.y, color = "blue")) +
  geom_errorbarh(aes(xmin = `Lower 0.95.y`, xmax = `Upper 0.95.y`, color = "blue"), 
                 height = 0.1) +
  # sensitivity analysis 2
  geom_point(aes(x = Effect.x.x, color = "green")) +
  geom_errorbarh(aes(xmin = `Lower 0.95.x.x`, xmax = `Upper 0.95.x.x`, color = "green"), 
                 height = 0.1) +
  # sensitivity analysis 3
  geom_point(aes(x = Effect.y.y, color = "grey50")) +
  geom_errorbarh(aes(xmin = `Lower 0.95.y.y`, xmax = `Upper 0.95.y.y`, color = "grey50"), 
                 height = 0.1) +
  # sensitivity analysis 4
  geom_point(aes(x = Effect.x.x.x, color = "orange")) +
  geom_errorbarh(aes(xmin = `Lower 0.95.x.x.x`, xmax = `Upper 0.95.x.x.x`, color = "orange"), 
                 height = 0.1) +
  # sensitivity analysis 5
  geom_point(aes(x = Effect.y.y.y, color = "purple")) +
  geom_errorbarh(aes(xmin = `Lower 0.95.y.y.y`, xmax = `Upper 0.95.y.y.y`, color = "purple"), 
                 height = 0.1) +
  # sensitivity analysis ses
  geom_point(aes(x = Effect, color = "red")) +
  geom_errorbarh(aes(xmin = `Lower 0.95`, xmax = `Upper 0.95`, color = "red"), 
                 height = 0.1) +
...
supfig2mod2 <- mod2s %>% rownames_to_column(., "Name") %>%
  ...
  left_join(., mod2sens5 %>% rownames_to_column(., "Name"), 
            by = c("Name", "Low", "High", "Diff.", "S.E.", "Type"))  %>%
  full_join(., mod2sensses %>% rownames_to_column(., "Name"), 
            by = c("Name", "Low", "High", "Diff.", "S.E.", "Type"))  %>%
  select(-Low, -High, -Diff., -S.E., -Type) %>% 
  column_to_rownames(., "Name") %>%
  mutate(cat = factor(c("Age", "Age", "Age",
...
```{r suptab5}
# make table comparing and contrasting missing and non-missing data in each analysis

## first need to add columns to "maindf" to denote if in each analysis
maindf <- maindf %>%
  mutate(mod1 = ifelse(!is.na(AntenatalVisits), "Included", "Excluded"),
         mod2 = ifelse(!is.na(ModAtt), "Included", "Excluded"),
         mod3 = ifelse(!is.na(DeliveryHealth), "Included", "Excluded"))

# now make and export three separate tables
table1::table1(~ as.factor(PregnancyNumber) + AgeAtDelivery + AntenatalVisits +
                 Education + Scholar + PregnancyPlanned + as.factor(ContBeMod) + ContraceBe +
                 as.factor(ContAfMod) + ContraceAf + DeliveryPlace + Attendant + 
                 Refugee + ResStatus | mod1, 
               data = maindf, overall = FALSE, 
               render.continuous = c(.="Median [Q1, Q3]"))
table1::table1(~ as.factor(PregnancyNumber) + AgeAtDelivery + AntenatalVisits +
                 Education + Scholar + PregnancyPlanned + as.factor(ContBeMod) + ContraceBe +
                 as.factor(ContAfMod) + ContraceAf + DeliveryPlace + Attendant + 
                 Refugee + ResStatus | mod2, 
               data = maindf, overall = FALSE, 
               render.continuous = c(.="Median [Q1, Q3]"))
table1::table1(~ as.factor(PregnancyNumber) + AgeAtDelivery + AntenatalVisits +
                 Education + Scholar + PregnancyPlanned + as.factor(ContBeMod) + ContraceBe +
                 as.factor(ContAfMod) + ContraceAf + DeliveryPlace + Attendant + 
                 Refugee + ResStatus | mod3, 
               data = maindf, overall = FALSE, 
               render.continuous = c(.="Median [Q1, Q3]"))
```
...
# combine
write.csv(cbind(mod1sup5, mod2tab, mod3tab), "./tabs/suptab6.csv", row.names = TRUE)
```
...
mod1s1tab <- mod1sens1 %>% 
  mutate(Mod1s1 = paste0(round(Effect, 2), 
                       ...
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod1s1) %>%
  add_row(Mod1s1 = NA, .after = 16)
mod1s2tab <- mod1sens2 %>% 
  mutate(Mod1s2 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod1s2) %>%
  add_row(Mod1s2 = NA, .after = 16)
mod1s3tab <- mod1sens3 %>% 
  mutate(Mod1s3 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod1s3) %>%
  add_row(Mod1s3 = NA, .after = 16)
mod1s4tab <- mod1sens4 %>% 
  mutate(Mod1s4 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod1s4) %>% add_row(Mod1s4 = NA, .after = 8) %>%
  add_row(Mod1s4 = NA, .after = 16)
mod1s5tab <- mod1sens5 %>% 
  mutate(Mod1s5 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod1s5) %>% add_row(Mod1s5 = NA, .after = 8) %>%
  add_row(Mod1s5 = NA, .after = 16)
mod1sstab <- mod1sensses %>% 
  mutate(Mod1ss = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod1ss) %>% add_row(Mod1ss = NA, .after = 4)

# need to update all with row for SES data
mod1tab <- mod1tab %>% add_row(Mod1 = NA, .after = 16)
rownames(mod1tab)[17] <- "Absolute Household Socioeconomic Status"

# need to update rownames
rownames(mod1s1tab) <- rownames(mod1tab)
rownames(mod1s2tab) <- rownames(mod1tab)
rownames(mod1s3tab) <- rownames(mod1tab)
rownames(mod1s4tab) <- rownames(mod1tab)
rownames(mod1s5tab) <- rownames(mod1tab)

write.csv(cbind(mod1tab, mod1s1tab, mod1s2tab, mod1s3tab, mod1s4tab, mod1s5tab, mod1sstab), 
...
mod2s1tab <- mod2sens1 %>% 
  mutate(Mod2s1 = paste0(round(Effect, 2), 
                       ...
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod2s1) %>%
  add_row(Mod2s1 = NA, .after = 18)
mod2s2tab <- mod2sens2 %>% 
  mutate(Mod2s2 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod2s2) %>%
  add_row(Mod2s2 = NA, .after = 18)
mod2s3tab <- mod2sens3 %>% 
  mutate(Mod2s3 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod2s3) %>%
  add_row(Mod2s3 = NA, .after = 18)
mod2s4tab <- mod2sens4 %>% 
  mutate(Mod2s4 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod2s4) %>% add_row(Mod2s4 = NA, .after = 10) %>%
  add_row(Mod2s4 = NA, .after = 18)
mod2s5tab <- mod2sens5 %>% 
  mutate(Mod2s5 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod2s5) %>% add_row(Mod2s5 = NA, .after = 10) %>%
  add_row(Mod2s5 = NA, .after = 18)
mod2sstab <- mod2sensses %>% 
  mutate(Mod2ss = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod2ss) %>% add_row(Mod2ss = NA, .after = 6)

# need to update all with row for SES data
mod2tab <- mod2tab %>% add_row(Mod2 = NA, .after = 18)
rownames(mod2tab)[19] <- "Absolute Household Socioeconomic Status"

# need to update rownames
rownames(mod2s1tab) <- rownames(mod2tab)
rownames(mod2s2tab) <- rownames(mod2tab)
rownames(mod2s3tab) <- rownames(mod2tab)
rownames(mod2s4tab) <- rownames(mod2tab)
rownames(mod2s5tab) <- rownames(mod2tab)

write.csv(cbind(mod2tab, mod2s1tab, mod2s2tab, mod2s3tab, mod2s4tab, mod2s5tab, mod2sstab), 
...
mod3s1tab <- mod3sens1 %>% 
  mutate(Mod3s1 = paste0(round(Effect, 2), 
                       ...
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod3s1) %>%
  add_row(Mod3s1 = NA, .after = 18)
mod3s2tab <- mod3sens2 %>% 
  mutate(Mod3s2 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod3s2) %>%
  add_row(Mod3s2 = NA, .after = 18)
mod3s3tab <- mod3sens3 %>% 
  mutate(Mod3s3 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod3s3) %>%
  add_row(Mod3s3 = NA, .after = 18)
mod3s4tab <- mod3sens4 %>% 
  mutate(Mod3s4 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod3s4) %>% add_row(Mod3s4 = NA, .after = 10) %>%
  add_row(Mod3s4 = NA, .after = 18)
mod3s5tab <- mod3sens5 %>% 
  mutate(Mod3s5 = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod3s5) %>% add_row(Mod3s5 = NA, .after = 10) %>%
  add_row(Mod3s5 = NA, .after = 18)
mod3sstab <- mod3sensses %>% 
  mutate(Mod3ss = paste0(round(Effect, 2), 
                       " (", round(`Lower 0.95`, 2), 
                       ", ", round(`Upper 0.95`, 2), ")")) %>%
  select(Mod3ss) %>% add_row(Mod3ss = NA, .after = 6)

# need to update all with row for SES data
mod3tab <- mod3tab %>% add_row(Mod3 = NA, .after = 18)
rownames(mod3tab)[19] <- "Absolute Household Socioeconomic Status"

# need to update rownames
rownames(mod3s1tab) <- rownames(mod3tab)
rownames(mod3s2tab) <- rownames(mod3tab)
rownames(mod3s3tab) <- rownames(mod3tab)
rownames(mod3s4tab) <- rownames(mod3tab)
rownames(mod3s5tab) <- rownames(mod3tab)

write.csv(cbind(mod3tab, mod3s1tab, mod3s2tab, mod3s3tab, mod3s4tab, mod3s5tab, mod3sstab), 
          "./tabs/suptab10.csv", row.names = TRUE)
```
